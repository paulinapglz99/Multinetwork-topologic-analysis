...
counts <- args[1]
metadata <- args[2]
counts <- "/datos/rosmap/data_by_counts/ROSMAP_counts/counts_by_tissue/DLFPC/full_counts/ROSMAP_RNAseq_rawcounts_DLPFC.rds"
metadata <- "/datos/rosmap/data_by_counts/ROSMAP_counts/counts_by_tissue/metadata/DLPFC/RNA_seq_metadata_DLPFC.txt"
...
#   #Check if the ‘exclude’ column has NA
#   if (any(is.na(metadata$exclude))) {
#     #If there are NA values, the NA values are converted to FALSE and the TRUE values are retained. 
#     metadata <- metadata %>% mutate(exclude = ifelse(is.na(exclude), FALSE, TRUE))
#   } else {
#     #If there is no NA, assume that the column has FALSE and TRUE
...
homogenize_exclude <- function(metadata) {
  ...
  if (!"exclude" %in% colnames(metadata)) {
    return(metadata)
  }
  
  metadata <- metadata %>%
    mutate(
      exclude = ifelse(is.na(exclude), FALSE, as.logical(exclude))
    )
...
pca <- function(counts, factors, title = "PCA Scatterplot coloured by sequencing batch", 
  ...
  #Save the graph if save_plot is TRUE
  if (save_plot) {
    ggsave(filename = filename, plot = plot, width = 10, height = 8, dpi = 300)
  }
  
  #Return the PCA dataframe and variance table
...
metadata <- metadata[metadata$sequencingBatch %in%  names(which(table(metadata$sequencingBatch) > 1)), ]
metadata <- metadata %>% filter(if_all(c(ceradsc, sex), ~ !is.na(.)))
table(metadata$sequencingBatch)
dim(metadata)
...
#metadata <- metadata[!(metadata$specimenID %in% c("hB_RNA_10892_resequenced", "hB_RNA_10892_K77C014")), ]
...
message("myannot table \n", 
        paste(capture.output(table(myannot$percentage_gene_gc_content, useNA = "ifany")), collapse = "\n"))
...
explo.plot(mycountsbio,
           ...
           samples = 1:12)  #only showing the first 15 samples
dev.off()
...
message("FeatureData columns")
print(colnames(featureData))

#sum(is.na(featureData$GC))
#sum(is.nan(featureData$GC))

message(paste("featureData GC table \n", 
              paste(capture.output(table(featureData$percentage_gene_gc_content, useNA = "ifany")), 
                    collapse = "\n")))
