...
#Read RDS counts
counts <- "/datos/rosmap/data_by_counts/Mayo_counts/counts_by_tissue/cerebellum/Mayo_RNAseq_rawcounts_CRB.rds"
counts <- readRDS(counts)
#Read metadata
metadata <- "/datos/rosmap/data_by_counts/Mayo_counts/counts_by_tissue/cerebellum/Mayo_RNAseq_metadata_CRB.txt"
...
pca <- function(counts, factors, title = "PCA Scatterplot coloured by sequencing batch", 
  ...
  plot <- pca_df %>% 
    ggplot() +
    aes(x = PC1, y = PC2, 
        colour = as.factor(factors$specimenIdSource)) +
    geom_point() +
...
metadata <- metadata[metadata$specimenIdSource %in%  names(which(table(metadata$specimenIdSource) > 1)), ]
metadata <- metadata %>% filter(if_all(c(diagnosis, sex), ~ !is.na(.)))
dim(metadata)
...
counts <- counts %>% mutate(feature = str_remove(feature, "\\..*$")) %>% as.data.frame()
...
#cols_exist <- intersect(metadata$specimenID, colnames(counts))
#counts <- counts[, c(1, intersect(metadata$specimenID, colnames(counts))), drop = FALSE]
#metadata <- metadata %>% dplyr::select(1, colnames(counts))
#counts <- counts %>% dplyr::select(1, all_of(metadata$specimenID))
valid_ids <- intersect(metadata$specimenID, colnames(counts))
counts <- counts %>% dplyr::select(1, all_of(valid_ids))
dim(counts)
...
x<- setNames(factors$specimenID, factors$diagnosis)
...
message("counts")
ncol(exprs(noiseqData_norm_count))
message("factors dim")
nrow(factors)
lessbatch <- ComBat_seq(counts = exprs(noiseqData_norm_count), batch = factors$specimenIdSource)
